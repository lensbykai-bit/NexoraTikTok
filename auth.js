...
function setAuthBusy(busy,label){
  ...
  if(busy){
    ...
    btn.textContent=btn.dataset.original||'Sign In';
    btn.disabled=false;
  }
}
...
function setLoginMode(mode){
  ...
  const passwordLabel=document.getElementById('passwordLabelText');
  const password=document.getElementById('password');
  const confirmWrap=document.getElementById('confirmFieldWrap');
  setAuthMessage('');

  if(mode==='signup'){
    if(title)title.textContent='Create Account';
    if(subtitle)subtitle.textContent='Create your Nexora student account.';
    ...
    if(title)title.textContent='Student Log In';
    if(subtitle)subtitle.textContent='Enter your details below.';
    if(nameWrap)nameWrap.hidden=true;
    if(confirmWrap)confirmWrap.hidden=true;
    if(submit)submit.textContent='Sign In';
    if(switchText)switchText.textContent='New to Nexora?';
    if(switchBtn){switchBtn.textContent='Create Account';switchBtn.hidden=false;}
...
async function handleLoginPage(){
  ...
  if(session&&getLoginMode()==='login'){
    setAuthMessage(`Signed in as ${session.user.email}. You can continue to the academy.`,'success');
    const submit=document.querySelector('.login-submit');
    if(submit)submit.textContent='Continue to Nexora';
    form.dataset.session='active';
  }

  document.getElementById('authSwitchBtn')?.addEventListener('click',()=>{
    setLoginMode(getLoginMode()==='signup'?'login':'signup');
  });
  ...
  document.getElementById('forgotPasswordBtn')?.addEventListener('click',async()=>{
    ...
    else setAuthMessage('Password reset email sent. Check your inbox.','success');
  });

  form.addEventListener('submit',async e=>{
    e.preventDefault();
    if(form.dataset.session==='active'){
    ...
    try{
      if(mode==='signup'){
        ...
        if(data.session){setAuthMessage('Account created. Welcome to Nexora!','success');setTimeout(()=>location.href='index.html',650);}
        else{setAuthMessage('Account created. Check your email to confirm your account, then sign in.','success');setLoginMode('login');}
      }else if(mode==='reset'){
        const {error}=await nexoraSupabase.auth.updateUser({password});
        if(error)throw error;
        setAuthMessage('Password updated. You are signed in.','success');
        setTimeout(()=>location.href='index.html',650);
      }else{
        const {error}=await nexoraSupabase.auth.signInWithPassword({email,password});
        if(error)throw error;
        setAuthMessage('Signed in successfully. Opening Nexora…','success');
      ...
      setAuthBusy(false);
      const submit=document.querySelector('.login-submit');
      if(submit&&mode==='signup')submit.textContent='Create Account';
      if(submit&&mode==='reset')submit.textContent='Update Password';
    }
  });

  nexoraSupabase.auth.onAuthStateChange((event)=>{
    if(event==='PASSWORD_RECOVERY')setLoginMode('reset');
  });
}
...
async function updateHeaderAuth(){
  ...
  loginLinks.forEach(link=>{
    if(session){
      ...
      link.title=session.user.email||'Signed in';
    }else{
      link.textContent=link.closest('.nav-actions')?'Log In':'Login';
      link.href='login.html';
      delete link.dataset.logout;
      link.removeAttribute('title');
    }
  });
  document.querySelectorAll('[data-logout="1"]').forEach(link=>link.addEventListener('click',async e=>{
    e.preventDefault();
    await nexoraSupabase.auth.signOut();
