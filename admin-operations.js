...
function money(e){if(e?.payment_amount==null)return '—';return `${e.payment_currency||'USD'} ${Number(e.payment_amount).toFixed(2)}`}
...
function renderOpsEnrollments(){
  ...
  const rows=opsEnrollments.filter(e=>{const search=!q||[e.full_name,e.email,e.track,e.goal,e.payment_reference,e.payment_provider].join(' ').toLowerCase().includes(q);let filter=true;if(f==='unmatched')filter=e.status==='approved'&&!studentForEnrollment(e);else if(f==='payment_pending')filter=e.payment_status==='pending';else if(f!=='all')filter=e.status===f;return search&&filter});
  if(!rows.length){root.innerHTML='<div class="ops-empty">No matching enrollments.</div>';return}
  root.innerHTML=rows.map(e=>{const matched=studentForEnrollment(e),payment=e.payment_status||'not_required';const cls=matched?'good':(e.status==='approved'?'warn':'');const payMeta=[e.payment_provider,money(e)!=='—'?money(e):''].filter(Boolean).join(' · ');return `<article class="ops-row"><div><strong>${oe(e.full_name)}</strong><span>${oe(e.email)}</span></div><div><strong>${oe(e.track)}</strong><small>${oe(enrollmentMatchLabel(e))}${payMeta?` · ${oe(payMeta)}`:''}</small></div><div><span class="ops-state ${cls}">${oe(e.status)} · ${oe(payment)}</span></div><button type="button" data-enrollment-id="${e.id}">Manage</button></article>`}).join('');
...
function openOpsEnrollment(id){
  opsCurrentEnrollment=opsEnrollments.find(e=>String(e.id)===String(id));if(!opsCurrentEnrollment)return;const e=opsCurrentEnrollment,s=studentForEnrollment(e);O('#opsEnrollmentTitle').textContent=e.full_name||'Enrollment';O('#opsEnrollmentDetails').innerHTML=[['Email',e.email],['Track',e.track],['Level',e.level],['Student match',s?`${s.display_name||'Student'} · ${s.email}`:'No matching portal account yet'],['Current access',s?.course_access||'—'],['Approved',od(e.approved_at)],['Matched',od(e.matched_at)],['Payment verified',od(e.payment_verified_at)],['Created',od(e.created_at)]].map(x=>detail(...x)).join('');O('#opsEditStatus').value=e.status||'new';O('#opsEditPayment').value=e.payment_status||'not_required';O('#opsEditPaymentProvider').value=e.payment_provider||'';O('#opsEditPaymentRef').value=e.payment_reference||'';O('#opsEditPaymentAmount').value=e.payment_amount??'';O('#opsEditPaymentCurrency').value=e.payment_currency||'USD';O('#opsEditNote').value=e.admin_note||'';O('#opsEnrollmentMessage').textContent='';O('#opsEnrollmentModal').classList.remove('hidden')
}
...
O('#opsSaveEnrollment')?.addEventListener('click',async()=>{
  ...
  const rawAmount=O('#opsEditPaymentAmount').value.trim(),amount=rawAmount===''?null:Number(rawAmount);if(amount!==null&&(!Number.isFinite(amount)||amount<0||amount>100000)){msg.textContent='Enter a valid payment amount.';return}
  const currency=(O('#opsEditPaymentCurrency').value.trim()||'USD').toUpperCase().slice(0,8);if(currency.length<3){msg.textContent='Currency must be at least 3 characters, such as USD or KHR.';return}
  btn.disabled=true;btn.textContent='Saving…';const payload={status,payment_status:payment,payment_provider:O('#opsEditPaymentProvider').value.trim(),payment_reference:O('#opsEditPaymentRef').value.trim(),payment_amount:amount,payment_currency:currency,payment_verified_at:payment==='paid'?(opsCurrentEnrollment.payment_verified_at||new Date().toISOString()):opsCurrentEnrollment.payment_verified_at,admin_note:O('#opsEditNote').value.trim(),updated_at:new Date().toISOString()};const{error}=await opsSb.from('nexora_enrollments').update(payload).eq('id',opsCurrentEnrollment.id);btn.disabled=false;btn.textContent='Save workflow';if(error){msg.textContent=error.message;return}msg.textContent=status==='approved'?'Saved. Automatic account matching/access grant is running ✓':'Workflow saved ✓';await loadOps();setTimeout(closeOpsEnrollment,700)
});
