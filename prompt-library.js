(()=>{
  ...
  if(!grid)return;
  let prompts=[],activePrompt='',visibleCount=PAGE_SIZE,failed=false;
  ...
  function filtered(){const q=(input?.value||'').trim().toLowerCase();return prompts.filter(p=>{const hay=`${p.title||''} ${p.category||''} ${p.prompt_text||''}`.toLowerCase();return !q||hay.includes(q)})}
  function render(){
  ...
  async function load(){
    failed=false;retry?.classList.add('hidden');grid.setAttribute('aria-busy','true');
    try{
      ...
      prompts=await res.json();visibleCount=PAGE_SIZE;render();wireCopy();
    }catch(err){console.error(err);prompts=[];failed=true;grid.innerHTML='<div class="prompt-load-error"><strong>Could not load the prompt library.</strong><span>Your connection or the content service may be temporarily unavailable.</span></div>';retry?.classList.remove('hidden');showMore?.classList.add('hidden');if(count)count.textContent='0'}finally{grid.setAttribute('aria-busy','false')}
  }
